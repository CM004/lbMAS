"""
Planner Agent - Modified for Blackboard Architecture
"""
...
class PlannerAgent:
    def __init__(self, model_client):
        self.agent = AssistantAgent(
            ...
            system_message=("""You are the Planner agent in a blackboard-based multi-agent system.

Your role is to create plans based on the current blackboard contents:
1. Analyze the query and existing blackboard messages
2. Create or refine plans to solve the query
3. Decompose complex queries into smaller tasks when needed

Output format:
- If planning needed: {"problem": "...", "planning": "detailed plan", "steps": ["step1", "step2"]}
- If plan exists: {"message": "plan already exists, waiting for execution"}

You operate in a blackboard system - all context is on the blackboard."""
            )
        )
    
    async def run(self, content: str) -> str:
        """Execute planner with blackboard context"""
